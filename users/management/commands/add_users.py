...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        payments = [
            ...
            {'user': user, 'paid_course': course, 'payment_amount': 100, 'payment_method': "Наличные"},
        ]

        for payment_data in payments:
            payment, created = Payment.objects.get_or_create(**payment_data)
            if created:
                self.stdout.write(self.style.SUCCESS(f'Successfully added payment: {user.email}'))
            else:
                self.stdout.write(self.style.WARNING(f'Payment already exists: {user.email}'))
